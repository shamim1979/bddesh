        ...
        $(this).each(function(){
			...
			t = t.replace(/(https\:\/\/|http:\/\/)([www\.]?)([^\s|<]+)/gi,function($1$2$3){
    			...
    			if (match && match[3]) {
    				...
    				return '<a class="conbor rdMr lbig" target="_blank" href="'+urlData+'" '+class_string+' '+rel_string+'><span aria-hidden="true" class="glyphicon glyphicon-link sml"></span> '+url_domain(urlData)+'</a>';
    			}
        	});   
			t = t.replace(/[#]+[A-Za-z0-9-_]+/g, function(u) {
				var u = u.replace("#","");
				return '<a class="brd3 hswa" href="/quoteDay/hashword/'+u+'">'+u+'</a>';
